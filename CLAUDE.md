...
5. **Ship via PR (standing policy, owner-approved 2026-07-16):** each task on its own branch (`t<task>-<slug>`) → push → `gh pr create` → wait for CI green (`gh pr checks --watch`) → `gh pr merge --squash --delete-branch` → pull main. Doc-only/infra-note commits may go straight to main. **After every merge that adds migrations, apply them to Neon dev AND production (standing policy, owner-approved 2026-07-16):** `npm run migrate -w apps/api` (dev, reads `.env`) then `npm run migrate:prod -w apps/api` (reads `PROD_DATABASE_URL` from gitignored `.env`; up-only by design — never run `migrate:down` against prod).
