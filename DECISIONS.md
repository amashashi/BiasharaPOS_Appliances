...
### D-039 — SMS goes through Beem Africa; the adapter renders the (bilingual) template
**Date:** 2026-07-19 · **Mode:** Builder (T5.4)
**Decision:** The `NotificationService` port is bound to a **Beem Africa** SMS adapter (`NOTIFICATION_MODE=beem`) — the leading Tanzanian SMS provider. The platform has no exposed SMS API (its `smsService.js` is an internal Twilio client), so like ClickPesa (D-035) this is a direct provider integration behind the port, isolated to one file. **Contract:** `POST https://apisms.beem.africa/v1/send`, `Authorization: Basic base64(api_key:secret_key)`, JSON `{ source_addr, encoding:0, message, recipients:[{recipient_id, dest_addr}] }` (dest_addr = `255…`, no `+`); success = `successful:true`/`code:100`. **The port carries a `templateKey`+params, not text, so the adapter renders the message** (`sms-templates.ts`): the three reminder templates (`reminder.upcoming|due|overdue`) in Swahili + English, defaulting to Swahili (`NOTIFICATION_LOCALE`) — TZ customers' language. `sendSms` throws on any provider failure so the reminder dispatcher (T3.4) logs FAILED and never silently drops a reminder. Config from `BEEM_*` env; tests keep the recording stub.
**Why:** SMS deliverability to TZ numbers is best through a local provider (the platform's own code notes switching off Twilio to AfricasTalking/Bongolive for production); Beem is the market leader with a clean API + sandbox. Rendering in the adapter honours the frozen port shape (templateKey+params) while keeping the message content out of the domain code.
**Rejected:** calling a platform SMS endpoint (none exists); Twilio (the platform's dev choice — global, pricier, weaker TZ deliverability; kept as a one-file swap option since the platform already has creds for staging); passing rendered text through the port (would break the frozen M0 contract); per-merchant template storage (deferred — sensible defaults for now).
**Status:** active · **Blocked:** the live verify (a reminder SMS received on a real TZ number) needs owner-provisioned Beem credentials (`api_key`, `secret_key`, registered `sender_id`) — same shape as T5.3's ClickPesa live sandbox. Adapter is faithful to docs.beem.africa and fake-server-verified.

---

### D-038 — Fiscal aging measures from when the sale happened (occurredAt), not when it synced
**Date:** 2026-07-19 · **Mode:** Builder (T5.7)
**Decision:** A payment now carries `occurredAt` — the wall-clock time the money was actually collected — separate from `at` (the ledger write time). For an online sale they coincide; for an offline sale replayed on reconnect, `occurredAt` is the offline sale time (the POS stamps `soldAt` into the outbox op; the sync threads it into `applyPayment`). The **fiscal aging alert** (`GET /fiscal/aging`, OWNER) lists payments with no VFD receipt whose `COALESCE(occurredAt, at)` is older than the TRA submission window (`FISCAL_TRA_WINDOW_HOURS`, default 24) — so a sale made offline two days ago and replayed today reads as ~2 days overdue, not fresh. A red back-office banner surfaces the count on every screen. `occurredAt` is **nullable** and read via `COALESCE(occurredAt, at)` because the payments ledger is append-only (a trigger blocks UPDATE, so pre-T5.7 rows can't be back-filled). Replayed payments already fiscalize through the normal enqueue→worker path (the T2.4 queue keeps failed jobs precisely for this), so "queued payments fiscalize on replay" needed only verification.
**Why:** TRA measures a receipt's lateness from the sale, not from the merchant's connectivity. Measuring aging from `at` (sync time) would blind the alert to exactly the offline sales the feature exists to protect — a long-offline device would replay stale sales that look fresh and silently blow the compliance window. The alert reads the DB (payments without receipts) rather than the BullMQ queue so it's correct regardless of queue/Redis state and covers both a fiscal-service outage and a late offline replay.
**Rejected:** measuring from `at`/sync time (blind to offline lateness — the whole point); a non-null `occurredAt` with a back-fill UPDATE (the append-only trigger forbids it); reading the BullMQ failed-job set for the alert (couples the compliance signal to queue internals and misses payments never enqueued). **Note:** the concrete TRA window is the open compliance question that also blocks T5.2 — `FISCAL_TRA_WINDOW_HOURS` is configurable and defaults to 24h until that answer lands.
**Status:** active

---

### D-037 — Offline sales replay conflict-tolerantly: the sale always lands, conflicts are flagged not forced
**Date:** 2026-07-19 · **Mode:** Builder (T5.6)
**Decision:** When an offline sale replays (T5.5's `/sync/outbox`), the order + cash **always land** — the sale physically happened, money was collected — and any clash with authoritative state is recorded as a `sync_exceptions` row for an OWNER to resolve, never forced or silently dropped (ARCHITECTURE offline note). Two kinds: **STALE_PRICE** (the created order line's price ≠ current catalog, i.e. the catalog moved since the cashier last synced — the order keeps the price the customer actually paid, flagged for review) and **SERIAL_CONFLICT** (a serial the POS scanned offline is no longer IN_STOCK for that product — sold online meanwhile). Detection runs only on the first replay (the create path), so exceptions never duplicate; a partial unique index `(orderId, kind, detail->>'serial') WHERE status='OPEN'` makes re-detection idempotent. **Resolution** (OWNER-only): SERIAL_CONFLICT → `reassign` an available serial (RESERVED to the order line via the existing `FulfillmentService.reserve`, which re-validates — the conflict is *corrected* into live state) or `acknowledge` (handle manually); STALE_PRICE → `accept` the paid price. The outbox op carries per-line `unitPriceTzs` (the offered price) and optional `serials`; the POS populates `unitPriceTzs` from the cart today, so stale-price fires from real offline sales.
**Why:** A completed cash sale is a fact — refusing to replay it or overwriting the online truth both lose money or history. Flag-and-resolve keeps one source of truth (the order ledger + the unit state machine) while surfacing exactly what a human must decide. Reusing `reserve()` for the fix means the correction goes through the same validated door as every other serial movement — no bespoke state write.
**Rejected:** failing the whole replay on a conflict (loses the sale + the cash record); auto-reassigning any available serial (a human must choose which physical unit the customer took); auto-repricing a stale-price order (changes the ledger the customer already paid against — `accept` records the decision instead); a shared reconciliation table with T5.3a payments (different domain, different resolve actions).
**Status:** active · **Deferred:** POS offline serial *capture* (scanning the unit into the outbox op) needs the offline stock snapshot — until then serial-conflict is exercised by tests + engineered replays; the outbox contract already carries `serials`.

---

### D-036 — Offline outbox: server-side clientRef idempotency + app-orchestrated replay (not SW Background Sync)
**Date:** 2026-07-18 · **Mode:** Builder (T5.5)
**Decision:** Offline cash sales queue in an IndexedDB outbox with a client-generated `clientRef` (uuid) and replay to `POST /sync/outbox` on reconnect. **Exactly-once is enforced server-side**, not client-side: `clientRef` is a partial-unique key on `sales_orders` (`UNIQUE(merchantId, clientRef) WHERE clientRef IS NOT NULL`), so a double-replay finds the existing order instead of creating a duplicate, and a half-finished replay (order created, payment not yet recorded) converges — the missing cash payment is applied on the next pass, gated on "order has no payment yet" so it never double-charges. Each operation in a batch succeeds/fails independently; a bad one never aborts the batch. **Replay is orchestrated by the app** (a single-flight `drainOutbox` fired on load + the `online` event), not by a Service Worker Background Sync: the realistic POS scenario is a terminal left open all day whose network flaps, so the loaded SPA + outbox covers it, and app-level replay is testable and has no SW/HMR interference. Scope: **cash sales only** — mobile money needs the live push and credit agreements need connectivity (ARCHITECTURE offline note). Server-side idempotency makes the client self-healing: even if a drain fails to clear the local queue (e.g. a transient 401→refresh on the first authenticated call after load), the next drain re-posts and the server dedupes — no duplicate orders, ever.
**Why:** Money is sacred, and the network is the least reliable part of the retterminal. Putting the exactly-once guarantee on the server (a unique key) rather than trusting the client to remove-after-ack means no failure mode — dropped response, double-fire, crash mid-replay — can double-charge or duplicate an order. Serial conflicts (T5.6) and offline fiscalization + aging alerts (T5.7) build on this; a replayed payment already fiscalizes through the normal queue.
**Rejected:** SW Background Sync replay (harder to test, HMR interference, and the app-tab is already the natural replay driver); client-only idempotency via remove-after-ack (a dropped response would re-post and duplicate); one-big-transaction composing OrdersService.create + PaymentsService.record (create manages its own transaction; the clientRef key + converge-on-replay is simpler and equally safe); offline mobile money / credit (impossible without connectivity).
**Status:** active · **Deferred:** SW app-shell caching for cold offline load (a PWA-packaging concern, M6); server-rejected outbox ops currently drop after logging — T5.6 routes them to the exception queue.
