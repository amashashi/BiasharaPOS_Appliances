...
- **T1.5 DONE — M1 COMPLETE** — `products.isSerialized` (immutable once stock exists) + `stock_levels` qty ledger; mixed GRNs (serials[] vs qty per line, type-shaped validation); stock view shows both models. 84 tests green. ⚠️ Neon **production** branch still needs migration 1784160000000 (guardrail deferred it; dev branch is current — apply at deploy or with explicit owner go).
...
- **➡ Next action: T2.1** — SalesOrder module: quote/order with lines + service lines (delivery/installation), status lifecycle, quote→order conversion, printable quote PDF. Starts M2.
