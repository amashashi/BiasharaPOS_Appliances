...
## Where we are
- **M0 in progress.** T0.1 ✅ scaffold+CI · T0.2 ✅ TypeORM data layer · T0.3 ✅ platform adapter contracts + stubs (9 tests green).
- **➡ Next action: T0.4** — auth middleware (validate platform JWT via IDENTITY_SERVICE, resolve merchant/user/roles, role guards OWNER/CASHIER/DELIVERY), T0.5 audit table, T0.6 design tokens (transcribe from `design-handoff/_ds/*/tokens/*.css`, bundle Plus Jakarta Sans TTFs), T0.7 brand assets (canonical SVGs in `brand/`, D-016).
