...
## Where we are
- **M0 in progress.** T0.1 ✅ (monorepo scaffold, CI, healthcheck verified) · T0.2 ✅ (TypeORM data layer: Merchant/Location/UserRef, SQL migrations, seed — verified migrate up/down/up + psql).
- **➡ Next action: T0.3** — define frozen platform adapter interfaces (`FiscalService`, `PaymentsService`, `IdentityService`, `NotificationService`) in `packages/shared` + stub implementations + contract tests. Then T0.4 auth guard, T0.5 audit table, T0.6 design tokens (transcribe from `design-handoff/_ds/*/tokens/*.css`, bundle Plus Jakarta Sans TTFs), T0.7 brand assets (canonical SVGs in `brand/`, D-016).
