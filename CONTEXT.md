...
- **➡ Next action: T2.4** — Fiscal integration point: every payment triggers `FiscalService.issueReceipt` (stub); receipt (HTML/PDF) with VFD number + QR; fiscal failures land in a BullMQ retry queue. Needs Redis — check availability on this machine first (D-017-style fallback may be needed).
