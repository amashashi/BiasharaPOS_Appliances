...
- **T1.1 DONE** — Catalog module: products table + CRUD (OWNER writes, any-auth reads, soft archive) + CSV import with per-row `{line, errors[]}` reporting (D-018); 50-row fixture at `apps/api/test/fixtures/products-sample-50.csv`. 36 tests green.
...
- **➡ Next action: T1.2** — GRN receiving: create receipt with lines; capture serials per line; duplicate-serial rejection per merchant. Then T1.3 serial state machine.
