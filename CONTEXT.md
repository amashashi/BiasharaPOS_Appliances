...
## Where we are
- **M0 in progress.** T0.1 ✅ scaffold+CI · T0.2 ✅ data layer · T0.3 ✅ adapter contracts+stubs · T0.4 ✅ auth guard+roles · T0.5 ✅ append-only audit log (16 tests green). Convention: explicit types on every @Column (vitest has no decorator metadata).
- **➡ Next action: T0.6** — design tokens + packages/ui foundation (transcribe from `design-handoff/_ds/*/tokens/*.css`, bundle Plus Jakarta Sans TTFs, hex-literal lint rule), T0.6 design tokens (transcribe from `design-handoff/_ds/*/tokens/*.css`, bundle Plus Jakarta Sans TTFs), T0.7 brand assets (canonical SVGs in `brand/`, D-016).
